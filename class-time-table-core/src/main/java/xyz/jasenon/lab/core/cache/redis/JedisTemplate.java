...
public  class JedisTemplate implements  Serializable{  
	...
	public  Jedis getJedis() {
		...
		try {
			...
			// 修复：使用 jedis.close() 替代已弃用的 returnBrokenResource
			if(jedis != null) {
				try {
					jedis.close();
				} catch (Exception ex) {
		...
		do {
			try {
				...
				logger.error("获取 Jedis 连接失败，重试次数: " + count, e);
			}
			count++;
	...
	public  void close(Jedis jedis) {
		// 修复：使用 jedis.close() 替代已弃用的 returnResource
		// Jedis 3.x+ 中 close() 会自动检测连接状态，正常归还或标记为损坏
		if (jedis != null) {
			try {
				jedis.close();
			} catch (Exception e) {
				logger.warn("关闭 Jedis 连接时发生异常", e);
			}
		}
	}
   ...
   abstract class Executor<T> { 
       ...
       public T getResult() {  
           ...
           try {  
               ...
               // 修复：使用 jedis.close() 替代 returnResource
               if (jedis != null) {  
                   try {
                       jedis.close();
                   } catch (Exception ex) {
                       logger.warn("关闭 Jedis 连接时发生异常", ex);
                   }
               }  
           }  
           return result;  

...
```mermaid
flowchart TD
  Q["Quartz Trigger"] --> J1["TaskGroupJob.executeInternal(groupId)"]
  J1 --> S1["TaskRuntimeService.execute(groupId)"]
  S1 --> C1["ConfigLoader.load(groupId)\n加载组/任务/条件/数据源"]
  C1 --> TW{"TimeWindowChecker.pass()"}
  TW -- 否 --> SKIP1["RunLogger.skipped：时间不满足"]
  TW -- 是 --> CE["ConditionEvaluator.evaluate()"]
  CE --> DS["DataSourceExecutor.fetch(dataSourceId, cacheTtl)\nHTTP/SQL/KV/CUSTOM 拉取并可缓存"]
  DS --> EXPR["ExpressionEvaluator.condition(expr, ctx)"]
  EXPR --> CE2["汇总逻辑 ALL/ANY → ConditionResult"]
  CE2 -- 未通过 --> SKIP2["RunLogger.skipped：条件不满足"]
  CE2 -- 通过 --> TE["TaskExecutor.runTasks(tasks, rc)"]
  TE --> GROUP["按 order_no 分组\n同组并行、组间串行"]
  GROUP --> PAR["runParallel(group)"]
  PAR --> RETRY["execWithRetry(task)\n重试/退避/超时/幂等 token"]
  RETRY --> EXEC["doExec(task, rc)\n分派 HTTP/SQL/MQ/CUSTOM 执行"]
  EXEC --> DETAIL["TaskExecDetail 记录成功/失败"]
  PAR --> DETAIL
  DETAIL --> RES["汇总 TaskResult"]
  RES --> RL["RunLogger.finish(groupId, traceId, start, result)"]
  RL --> ALRT{"是否失败？"}
  ALRT -- 是 --> NOTI["AlertService.notify(alertCfg, summary)\nSMS/SMTP 通道节流+连续失败判断"]
  ALRT -- 否 --> DONE["结束"]
  SKIP1 --> DONE
  SKIP2 --> DONE
```
...
关键类与作用：
- TaskGroupJob：Quartz Job，入口，提取 groupId 调用 TaskRuntimeService。
- TaskRuntimeService.execute：编排全链路，生成 traceId，加载配置，时间窗校验、条件评估、任务执行、汇总日志。
- TimeWindowChecker.pass：校验业务时段/工作日/节假日/日期区间。
- ConditionEvaluator.evaluate：遍历条件，使用 DataSourceExecutor 拉数据，ExpressionEvaluator 评估表达式，按逻辑汇总。
- DataSourceExecutor.fetch：按类型执行 HTTP/SQL/KV/CUSTOM，支持缓存、超时、白名单。
- TaskExecutor.runTasks：按 order_no 分组串行，组内并行；execWithRetry 实现重试/退避；doExec 按类型实际执行。
- RunLogger.finish/skipped：写 run_log/condition_log/task_detail，构建 RunSummary。
- AlertService.notify：基于告警策略和连续失败/节流规则触发 SMS/SMTP。